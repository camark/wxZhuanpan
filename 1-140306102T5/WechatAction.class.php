...
class WechatAction extends Action {
	...
	private function getSubscribe($openid = ''){
		$re = "";
		$re .= "您好，欢迎关注XXXXX微信公众号。为了让您能方便快捷的使用XXXXX为您带来的服务，";
		...
		$re .= "• 快速浏览适合我的任务\n";
		$re .= "• 任务检索\n";
		$re .= "<a href='http://www.马赛克.com/wechat/oauth/?openid={$openid}'>点击立即进行绑定</a>";
	...
	public function oauth() {
		if (IS_POST) {
			...
			if (empty ( $username ))	 $this->message ( '请输入您注册的邮箱' );
			if (empty ( $password ))	 $this->message ( '请输入您的登陆密码' );
			if (empty ( $openid ))		 $this->message ( '参数错误' );
			if (! isEmail ( $username )) $this->message ( '请输入正确的邮箱地址' );
			
			$user = D ( 'Member' )->login ( $username, $password );
			if ($user == - 1) {
				$this->message ( '用户不存在' );
			} else if ($user == - 2) {
				$this->message ( '密码错误' );
			} else if ($user > 0) {
				$weuserModel = M ( 'Wechat_user' );
				$wechatuser = $weuserModel->where ( "openid = '{$openid}'" )->find ();
				if ($wechatuser) {
					
					if ($wechatuser ['oauth'] == 3) {
						$this->message ( '该帐号已经绑定' );
					}
					else {
						$wechatuser ['oauth'] = 3;
						$wechatuser ['status'] = 3;
						$wechatuser ['userid'] = $user;
						$wechatuser ['edittime'] = time();
						$we = $weuserModel->data($wechatuser)->save ();
						if ($we) {
					...
					/* 如果从数据库没查到资料重新从微信查询 */
					$weuser = $this->getuser ( $openid );
					if ($weuser) {
						$weuser['status'] = 3;//关注状态
						$weuser['oauth'] = 3;//绑定状态
						$weuser['userid'] = $user;
						$weuser['addtime'] = time();//第一次关注的时间
						$re = $weuserModel->data($weuser)->add();
						if ($re) {
							// 这里微信响应一条（恭喜您验证成功！您可以使用自定义菜单或直接回复信息查询想要了解的内容）信息；
							$this->send ( "恭喜您验证成功！您可以使用自定义菜单或直接回复信息查询想要了解的内容!", $openid, 'text' );
	...
	private function getCookieUserId($openid = NULL) {
		...
		if ($auth) { // 存在cookie
			...
			$weuser = M ( 'Wechat_user' )->where ( "openid = '{$openid}' AND oauth = 3" )->find (); // 查到userid
